<x-app-layout>
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold mt-2 text-gray-900 dark:text-white">Edit Product</h2>
    </div>
        ...
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-600 dark:bg-gray-700 dark:text-white focus:outline-none transition-all">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Price ($)</label>
                    <input type="number" step="0.01" name="price" value="{{ old('price', $product->price) }}" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-600 dark:bg-gray-700 dark:text-white focus:outline-none transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-600 dark:bg-gray-700 dark:text-white focus:outline-none transition-all">{{ old('description', $product->description) }}</textarea>
            </div>
            ...
            <div>
                 ...
                 @if($product->image)
                    <div class="mb-2">
                        <img src="{{ Storage::url($product->image) }}" class="w-20 h-20 rounded-xl object-cover border border-gray-200 dark:border-gray-700">
                    </div>
                 @endif
            ...
            <div class="space-y-3">
                <div class="flex items-center">
                    <input type="checkbox" name="is_available" id="is_available" value="1" {{ $product->is_available ? 'checked' : '' }} class="w-5 h-5 text-blue-600 border-gray-300 dark:border-gray-600 rounded focus:ring-blue-600 bg-white dark:bg-gray-700 cursor-pointer">
                    <label for="is_available" class="ml-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">Available for ordering</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_spicy" id="is_spicy" value="1" {{ $product->is_spicy ? 'checked' : '' }} class="w-5 h-5 text-red-500 border-gray-300 dark:border-gray-600 rounded focus:ring-red-500 bg-white dark:bg-gray-700 cursor-pointer">
                    <label for="is_spicy" class="ml-2 text-sm text-gray-700 dark:text-gray-300 font-medium cursor-pointer">🌶️ Spicy</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_vegetarian" id="is_vegetarian" value="1" {{ $product->is_vegetarian ? 'checked' : '' }} class="w-5 h-5 text-green-500 border-gray-300 dark:border-gray-600 rounded focus:ring-green-500 bg-white dark:bg-gray-700 cursor-pointer">
