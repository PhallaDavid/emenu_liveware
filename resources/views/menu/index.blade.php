    ...
    <div x-data="menuApp('{{ $table ?? 1 }}')">
        ...
        <header class="sticky top-0 z-40 bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-100">
            ...
            <div class="bg-gray-50 border-b border-gray-100 px-4 py-1.5 flex justify-end gap-3 max-w-md mx-auto">
                <a href="{{ route('lang.switch', 'km') }}" class="text-[10px] font-black uppercase tracking-widest {{ app()->getLocale() === 'km' ? 'text-black' : 'text-gray-400 hover:text-black' }}">KH</a>
                <div class="h-3 w-[1px] bg-gray-200 mt-0.5"></div>
                <a href="{{ route('lang.switch', 'en') }}" class="text-[10px] font-black uppercase tracking-widest {{ app()->getLocale() === 'en' ? 'text-black' : 'text-gray-400 hover:text-black' }}">EN</a>
            </div>
            ...
            <div class="max-w-md mx-auto px-4 py-3 flex items-center justify-between gap-4">
                <div class="flex-shrink-0">
                    <button @click="callWaiter()" 
                        ...
                        <svg class="w-4 h-4" :class="waiterCalled ? 'animate-bounce' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                        <span class="text-[10px] font-bold uppercase tracking-widest hidden sm:inline" x-text="waiterCalled ? '{{ __('messages.coming') }}' : '{{ __('messages.waiter') }}'"></span>
                    </button>
                </div>
                
                <div class="text-center flex-1">
                    <h1 class="font-bold text-base text-gray-800 leading-none">{{ __('messages.menu') }}</h1>
                    <p class="text-[10px] text-gray-500 mt-0.5">{{ __('messages.table') }} <span x-text="table"></span></p>
                </div>
                ...
                <div class="relative flex-shrink-0">
                    ...
                    <button x-show="search.length > 0" @click="search = ''" class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400">
                    ...
                    <div class="swiper-wrapper">
                        ...
                        <div class="swiper-slide !w-auto">
                            <a href="#category-{{ $category->id }}" 
                               ...
                               :class="activeCategory === {{ $category->id }} ? 'bg-primary text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 active:scale-95'"
            ...
            <template x-if="lastOrderId && orderStatus !== 'paid'">
                <div class="max-w-md mx-auto px-4 pb-3">
                    <div class="bg-primary text-white p-4 rounded-2xl shadow-xl flex items-center justify-between overflow-hidden relative group">
                        <!-- Animated background accent -->
                        <div class="absolute inset-0 bg-gradient-to-r from-primary/20 to-transparent opacity-50 group-hover:opacity-75 transition-opacity"></div>
                        ...
                        <div class="flex items-center gap-4 relative z-10">
                            <div class="relative">
                                ...
                                <div class="absolute inset-0 w-3 h-3 rounded-full" 
                                     :class="orderStatus === 'pending' ? 'bg-yellow-400' : (orderStatus === 'completed' ? 'bg-green-500' : 'bg-red-500')"></div>
                            </div>
                            
                            <div>
                                <h4 class="text-xs font-black uppercase tracking-widest opacity-60 leading-none mb-1">{{ __('messages.live_update') }}</h4>
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex items-center gap-1">
                                        @if($product->is_spicy)
                                        <span class="text-[10px]" title="{{ __('messages.spicy') }}">🌶️</span>
                                        @endif
                                        @if($product->is_vegetarian)
                                        <span class="text-[10px]" title="{{ __('messages.vegetarian') }}">🥬</span>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    <template x-if="getItemQty({{ $product->id }}) > 0">
                                        <div class="flex items-center gap-2 bg-gray-100 rounded-full px-2 py-1">
                                            <button @click="removeFromCart({{ $product->id }})" class="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-black font-bold">-</button>
                                            <span class="text-sm font-bold w-4 text-center" x-text="getItemQty({{ $product->id }})"></span>
                                            <button @click="addToCart({{ $product->toJson() }})" class="w-6 h-6 flex items-center justify-center text-gray-800 hover:text-black font-bold">+</button>
                                        </div>
                                    </template>
                                    
                                    <template x-if="getItemQty({{ $product->id }}) === 0">
                                        <button @click="addToCart({{ $product->toJson() }})" class="bg-primary text-white rounded-full w-8 h-8 flex items-center justify-center hover:scale-105 transition-transform active:scale-95">
        ...
        <div x-show="cart.length > 0" 
            ...
            <button @click="cartOpen = true" class="pointer-events-auto bg-primary text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 hover:scale-105 transition-transform active:scale-95 relative">
                <!-- Item Count Badge -->
                <div class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold shadow-lg animate-pulse" x-text="cartCount"></div>
        ...
        <div x-show="cartOpen" 
            ...
            <div x-show="cartOpen"
                 ...
                 <div class="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                     <div class="flex items-center gap-2">
                         <button x-show="checkoutStep === 2" @click="checkoutStep = 1" class="p-1 hover:bg-gray-200 rounded-full transition-colors">
                             <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                         </button>
                         <h2 class="text-lg font-bold" x-text="checkoutStep === 1 ? '{{ __('messages.view_order') }}' : '{{ __('messages.payment_method') }}'"></h2>
                     </div>
                 ...
                 <div class="flex-1 overflow-y-auto p-4">
                     ...
                     <div x-show="checkoutStep === 1" class="space-y-4">
                        <template x-for="(item, index) in cart" :key="index">
                            <div class="flex justify-between items-center bg-white border border-gray-100 p-3 rounded-xl shadow-sm">
                                ...
                                <div class="flex items-center gap-3 bg-gray-50 p-1 rounded-full">
                                    <button @click="changeQty(index, -1)" class="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center hover:bg-gray-100 text-gray-500 transition-colors">-</button>
                                    <span x-text="item.qty" class="font-bold w-4 text-center text-sm"></span>
                                    <button @click="changeQty(index, 1)" class="w-8 h-8 rounded-full bg-primary text-white shadow-md flex items-center justify-center hover:opacity-90 transition-opacity font-bold">+</button>
                                </div>
                            </div>
                        </template>
                        ...
                        <div class="mt-6">
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{{ __('messages.special_instructions') }}</label>
                     ...
                     <div x-show="checkoutStep === 2" class="space-y-6 py-2">
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{{ __('messages.order_type') }}</label>
                            <div class="relative group">
                                ...
                                <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-gray-300 group-focus-within:text-primary transition-colors">
                        ...
                        <div>
                            <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">{{ __('messages.payment_method') }}</label>
                            <div class="relative group">
                        ...
                        <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                            <div class="flex justify-between items-center mb-2">
                                <span class="text-xs text-gray-400 font-bold uppercase tracking-wider">{{ __('messages.total') }}</span>
                                <span class="text-2xl font-black text-gray-900" x-text="'$' + cartTotal"></span>
                            </div>
                            <div class="flex items-center gap-2 text-[10px] text-primary font-bold uppercase tracking-tighter">
        ...
        <div x-show="orderSuccess" 
             ...
             <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
                 ...
                 <button @click="orderSuccess = false; cartOpen = false;" class="bg-primary text-white px-6 py-2 rounded-lg font-medium">{{ __('messages.okay') }}</button>
             </div>
        </div>
            ...
            return {
                ...
                callWaiter() {
                    ...
                    fetch('{{ route("order.call-waiter") }}', {
                        ...
                        body: JSON.stringify({ table_number: this.table })
                    })
                    .then(res => res.json())
                    .then(data => {
                        if (data.success) {
                            ...
                            toast.className = 'fixed bottom-24 left-1/2 -translate-x-1/2 bg-black text-white px-6 py-3 rounded-2xl shadow-xl z-[100] animate-bounce text-sm font-bold';
